...
### Start with FootR 1, as Template
...
```{r load_data}
sheets <- excel_sheets("/home/wburr/Rushali_Analysis/New_Stuff/Foot R/Foot R.xlsx")  # note: spaces in names
FootR1 <- lapply(sheets, FUN = function(x) {
           read_xlsx(path = "/home/wburr/Rushali_Analysis/New_Stuff/Foot R/Foot R.xlsx",
                     sheet = x) })
...
```{r merge_sheets}
FootR1m <- do.call("rbind", FootR1)
names(FootR1m) <- c("PeakNum", "Sample", "Name", "Formula", "RTs", "1stRT","2ndRT","Similarity",
...
RT1 <- as.numeric(
         unlist(
           lapply(strsplit(FootR1m$RTs, ","), FUN = "[[", 1)
         )
       )
RT2 <- as.numeric(
         unlist(
           lapply(strsplit(FootR1m$RTs, ","), FUN = "[[", 2)
         )
       )
FootR1m <- data.frame(FootR1m[, c(1:4, 6:17)], RT1 = RT1, RT2 = RT2)
```

```{r samples}
unique(FootR1m$Sample)
save(file = "/home/wburr/Rushali_Analysis/New_Stuff/Foot R/FootR.rda", FootR1m)
```
