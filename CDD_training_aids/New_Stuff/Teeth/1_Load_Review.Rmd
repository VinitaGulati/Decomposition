...
### Start with Teeth 1, as Template

Teeth 1 has many sheets, each of which is a nicely organized listing of VOCs
as detected by the GCxGC system's built-in scanning software, and exported. 
We will load them all. 

```{r load_data}
sheets <- excel_sheets("/home/wburr/Rushali_Analysis/New_Stuff/Teeth/Teeth1.xlsx")  # note: spaces in names
Teeth1 <- lapply(sheets, FUN = function(x) {
           read_xlsx(path = "/home/wburr/Rushali_Analysis/New_Stuff/Teeth/Teeth1.xlsx",
                     sheet = x) })
...
of the lists of compounds relevant to Teeth 1, and merge them into a single
...
```{r merge_sheets}
Teeth1m <- do.call("rbind", Teeth1)
names(Teeth1m) <- c("PeakNum", "Sample", "Name", "Formula", "RTs", "1stdim", "2nd dim"," Similarity","Area", "Height", "QuantMass", "BaseMass", "QuantSN", "PeakSN", "Type", "Conc", "ConcUnits")
```
...
RT1 <- as.numeric(
         unlist(
           lapply(strsplit(Teeth1m$RTs, ","), FUN = "[[", 1)
         )
       )
RT2 <- as.numeric(
         unlist(
           lapply(strsplit(Teeth1m$RTs, ","), FUN = "[[", 2)
         )
       )
Teeth1m <- data.frame(Teeth1m[, c(1:4, 6:17)], RT1 = RT1, RT2 = RT2)
```

```{r samples}
unique(Teeth1m$Sample)
save(file = "/home/wburr/Rushali_Analysis/New_Stuff/Teeth/Teeth1.rda", Teeth1m)
```
