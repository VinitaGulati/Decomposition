...
#  * sample_names: names of specific samples (e.g., SS5_FootS_1_a)
#  * control_names: names of specific control samples (e.g., SS5_FootS_1_Control_a)
...
# Working Through All the FootS

```{r FootS}
load("/home/wburr/Rushali_Analysis/New_Stuff/FootS/FootSn.rda")
FootS1m$PeakSN<-as.numeric(FootS1m$PeakSN)
FootS1m$Sample <- str_replace_all(FootS1m$Sample, " ", "_")

FootS_SS3 <- join_common( clean_common(FootS1m,
                sample_names = c("SS3FootS_a", "SS3FootS_b", "SS3FootS_c"),
                control_names = c("SS4FootS_Control_a","SS4FootS_Control_b" ),
                ratio_par = 2.0) )
```
...
```{r}
all_SS <- vector("list", 1)
all_SS[[1]] <- FootS_SS3
names(all_SS) <- c(paste0("FootS_SS3"))
save(file = "/home/wburr/Rushali_Analysis/New_Stuff/FootS/all_SS_PCA.rda", all_SS)
rm(list = ls())
load("/home/wburr/Rushali_Analysis/New_Stuff/FootS/all_SS_PCA.rda")
```
...
So there are 78 unique compounds present across the 1
total sample, after Controlling (but with the 2.0 ratio
...
names(test)[1]
write.xlsx(x = test[[1]],
           file = "/home/wburr/Rushali_Analysis/New_Stuff/FootS/all_FootS_cleaned.xlsx",
...
Then, let's create a set of vectors, 1 in total.
...
extracting <- as.data.frame(as.numeric(extracting))
names(extracting) <- "SS3"  # manual
#extracting <- apply(extracting, MAR = 2, FUN = function(x) { as.numeric(x) })
row.names(extracting) <- pca_dat$Name
pca_dat <- extracting

write.csv(file = "pca_ready_FootS.csv", pca_dat)
```
