...
### Start with Donor1, as Template

Donor 1 has many sheets, each of which is a nicely organized listing of VOCs
as detected by the GCxGC system's built-in scanning software, and exported. 
We will load them all. 

```{r load_data}
sheets <- excel_sheets("/home/wburr/Rushali_REST/Donor_dat/D1.xlsx")  # note: spaces in names
D1 <- lapply(sheets, FUN = function(x) {
           read_xlsx(path = "/home/wburr/Rushali_REST/Donor_dat/D1.xlsx",
                     sheet = x) })
...
of the lists of compounds relevant to Donor 1, and merge them into a single
...
```{r merge_sheets}
D1m <- do.call("rbind", D1)
names(D1m) <- c("PeakNum", "Sample", "Name", "Formula", "RTs", "1stRT","2ndRT","Similarity",
...
RT1 <- as.numeric(
         unlist(
           lapply(strsplit(D1m$RTs, ","), FUN = "[[", 1)
         )
       )
RT2 <- as.numeric(
         unlist(
           lapply(strsplit(D1m$RTs, ","), FUN = "[[", 2)
         )
       )
D1m <- data.frame(D1m[, c(1:4, 6:17)], RT1 = RT1, RT2 = RT2)
```

```{r samples}
unique(D1m$Sample)
save(file = "/home/wburr/Rushali_REST/Donor1/D1.rda", D1m)
```
