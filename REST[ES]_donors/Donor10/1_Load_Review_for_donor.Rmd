...
### Start with Donor10, as Template

Donor 10 has many sheets, each of which is a nicely organized listing of VOCs
as detected by the GCxGC system's built-in scanning software, and exported. 
We will load them all. 

```{r load_data}
sheets <- excel_sheets("/home/wburr/Rushali_REST/Donor_dat/D10.xlsx")  # note: spaces in names
D10 <- lapply(sheets, FUN = function(x) {
           read_xlsx(path = "/home/wburr/Rushali_REST/Donor_dat/D10.xlsx",
                     sheet = x) })
...
of the lists of compounds relevant to Donor 10, and merge them into a single
...
```{r merge_sheets}
D10m <- do.call("rbind", D10)
names(D10m) <- c("PeakNum", "Sample", "Name", "Formula", "RTs", "1stRT","2ndRT","Similarity",
...
RT1 <- as.numeric(
         unlist(
           lapply(strsplit(D10m$RTs, ","), FUN = "[[", 1)
         )
       )
RT2 <- as.numeric(
         unlist(
           lapply(strsplit(D10m$RTs, ","), FUN = "[[", 2)
         )
       )
D10m <- data.frame(D10m[, c(1:4, 6:17)], RT1 = RT1, RT2 = RT2)
```

```{r samples}
unique(D10m$Sample)
save(file = "/home/wburr/Rushali_REST/Donor10/D10.rda", D10m)
```
