...
### Start with Donor4, as Template

Donor 4 has many sheets, each of which is a nicely organized listing of VOCs
as detected by the GCxGC system's built-in scanning software, and exported. 
We will load them all. 

```{r load_data}
sheets <- excel_sheets("/home/wburr/Rushali_REST/Donor_dat/D4.xlsx")  # note: spaces in names
D4 <- lapply(sheets, FUN = function(x) {
           read_xlsx(path = "/home/wburr/Rushali_REST/Donor_dat/D4.xlsx",
                     sheet = x) })
...
of the lists of compounds relevant to Donor 4, and merge them into a single
...
```{r merge_sheets}
D4m <- do.call("rbind", D4)
names(D4m) <- c("PeakNum", "Sample", "Name", "Formula", "RTs", "1stRT","2ndRT","Similarity",
...
RT1 <- as.numeric(
         unlist(
           lapply(strsplit(D4m$RTs, ","), FUN = "[[", 1)
         )
       )
RT2 <- as.numeric(
         unlist(
           lapply(strsplit(D4m$RTs, ","), FUN = "[[", 2)
         )
       )
D4m <- data.frame(D4m[, c(1:4, 6:17)], RT1 = RT1, RT2 = RT2)
```

```{r samples}
unique(D4m$Sample)
save(file = "/home/wburr/Rushali_REST/Donor4/D4.rda", D4m)
```
