...
### Start with Donor6, as Template

Donor 6 has many sheets, each of which is a nicely organized listing of VOCs
as detected by the GCxGC system's built-in scanning software, and exported. 
We will load them all. 

```{r load_data}
sheets <- excel_sheets("/home/wburr/Rushali_REST/Donor_dat/D6.xlsx")  # note: spaces in names
D6 <- lapply(sheets, FUN = function(x) {
           read_xlsx(path = "/home/wburr/Rushali_REST/Donor_dat/D6.xlsx",
                     sheet = x) })
...
of the lists of compounds relevant to Donor 6, and merge them into a single
...
```{r merge_sheets}
D6m <- do.call("rbind", D6)
names(D6m) <- c("PeakNum", "Sample", "Name", "Formula", "RTs", "1stRT","2ndRT","Similarity",
...
RT1 <- as.numeric(
         unlist(
           lapply(strsplit(D6m$RTs, ","), FUN = "[[", 1)
         )
       )
RT2 <- as.numeric(
         unlist(
           lapply(strsplit(D6m$RTs, ","), FUN = "[[", 2)
         )
       )
D6m <- data.frame(D6m[, c(1:4, 6:17)], RT1 = RT1, RT2 = RT2)
```

```{r samples}
unique(D6m$Sample)
save(file = "/home/wburr/Rushali_REST/Donor6/D6.rda", D6m)
```
