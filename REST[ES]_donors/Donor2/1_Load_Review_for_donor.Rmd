...
### Start with Donor2, as Template

Donor 2 has many sheets, each of which is a nicely organized listing of VOCs
as detected by the GCxGC system's built-in scanning software, and exported. 
We will load them all. 

```{r load_data}
sheets <- excel_sheets("/home/wburr/Rushali_REST/Donor_dat/D2.xlsx")  # note: spaces in names
D2 <- lapply(sheets, FUN = function(x) {
           read_xlsx(path = "/home/wburr/Rushali_REST/Donor_dat/D2.xlsx",
                     sheet = x) })
...
of the lists of compounds relevant to Donor 2, and merge them into a single
...
```{r merge_sheets}
D2m <- do.call("rbind", D2)
names(D2m) <- c("PeakNum", "Sample", "Name", "Formula", "RTs", "1stRT","2ndRT","Similarity",
...
RT1 <- as.numeric(
         unlist(
           lapply(strsplit(D2m$RTs, ","), FUN = "[[", 1)
         )
       )
RT2 <- as.numeric(
         unlist(
           lapply(strsplit(D2m$RTs, ","), FUN = "[[", 2)
         )
       )
D2m <- data.frame(D2m[, c(1:4, 6:17)], RT1 = RT1, RT2 = RT2)
```

```{r samples}
unique(D2m$Sample)
save(file = "/home/wburr/Rushali_REST/Donor2/D2.rda", D2m)
```
